...
`products` テーブルに以下のカラムを追加してください。

| カラム名    | 型   | 制約   |
|------------|------|--------|
| description | TEXT | —      |

追加後、`\d products` でカラムが追加されていることを確認してください。
確認後は `ALTER TABLE products DROP COLUMN description;` で元に戻してください。
...
1. `orders` テーブルの `product_id` カラムに `idx_orders_product_id` という名前のインデックス
2. `orders` テーブルの `status` カラムに `idx_orders_status` という名前のインデックス
...
**ファイル:** `exercises/chapter08/ex05.sql`

8-1、8-2 で作成した `products` と `orders` を削除してください。

- `orders` は `products` を参照しているため、削除順に注意してください
- テーブルが存在しない場合でもエラーにならないよう `IF EXISTS` を使ってください

削除後、`\dt` でテーブルが消えていることを確認してください。
